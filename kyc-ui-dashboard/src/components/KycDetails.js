...
const KycDetails = () => {
  ...
  useEffect(() => {
    const fetchKycDetails = async () => {
      ...
      const sortedData = actionsListResponse.sort((a, b) => a.action_id - b.action_id); // Sort ascending initially
    ...
    const handleSortById = () => {
      const sortedData = [...actionsList].sort((a, b) =>
        sortOrder === "asc" ? b.action_id - a.action_id : a.action_id - b.action_id
      );
  ...
  const currentMilestoneIndex = statuses.indexOf(capitalizeFirstLetter(kycDetail.status) || "");
  ...
  return (
    ...
    <div className="flex items-center space-x-3">
      ...
      <span className="text-gray-700">{kycDetail?.client_name || "N/A"}</span>
    </div>

    {/* Policy Name */}
    <div className="flex items-center space-x-3">
      <span className="text-gray-500 font-semibold">📜 Policy:</span>
      <span className="text-gray-700">{kycDetail?.policy_name || "N/A"}</span>
    </div>

    {/* Trigger Date */}
    <div className="flex items-center space-x-3">
      <span className="text-gray-500 font-semibold">📅 Trigger Date:</span>
      <span className="text-gray-700">{kycDetail?.trigger_date || "N/A"}</span>
    </div>
    ...
    <div className="flex items-center space-x-3 col-span-1 md:col-span-2">
      ...
      <span
        className={`px-3 py-1 rounded-lg text-white text-sm font-semibold
          ${kycDetail?.status === "Completed" ? "bg-green-500" :
            kycDetail?.status === "Verification" ? "bg-yellow-500" :
            "bg-red-500"}
        `}
      >
        ...
        <div className="mt-6">
          ...
          <div className="relative w-full h-1 bg-gray-300 mt-2">
            <div
              ...
              style={{ width: `${(currentMilestoneIndex / (statuses.length - 1)) * 100}%` }}
            ></div>
          </div>
        </div>
      </div>
          ...
          <tbody>
            {actionsList?.map((action, index) => (
              ...
              >
                ...
                <td className="px-4 py-2 border">{action.action_description}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {isModalOpen && selectedRequest && (
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg w-[80%] max-w-3xl">
          <h3 className="text-xl font-semibold mb-4 text-center">KYC Request Details</h3>
          ...
          <table className="w-full border-collapse border border-gray-300">
            <tbody>
              {[
                ...
                ["Client Evidence Source", selectedRequest?.client_evidence_source],
                ...
                ["External Evidence Source", selectedRequest?.external_evidence_source?.join(", ")],
                ["Internal Evidence Source", selectedRequest?.internal_evidence_source?.join(", ")],
                ["Latest Action Activity", selectedRequest?.latest_action_activity],
                ["Policy Quote", selectedRequest?.policy_quote],
                ["Role", selectedRequest?.role]
              ].map(([label, value], index) => (
                <tr key={index} className="border border-gray-300">
                  <td className="px-4 py-2 font-semibold bg-gray-200">{label}</td>
