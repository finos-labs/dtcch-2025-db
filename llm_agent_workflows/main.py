...
def main(kyc_id, policy_id, client_id):
    
    #PDF file path for the policy
    policy_document_path = fetch_policy_file_path(policy_id=policy_id)
    
    #TXT file path for internal client information for initial background check
    client_data_file_path = fetch_client_data_file_path(client_id=client_id)
    ...
    #After agent workflow 1, uncomment and call this to store the output json to database
    # update_action_in_progress(json_payload)
